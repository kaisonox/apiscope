...
static void AddHook(std::vector<HookDefinition>* hooks, const HookDefinition& hook) {
    auto existing = std::find_if(
        hooks->begin(),
        hooks->end(),
        [&hook](const HookDefinition& candidate) {
            return candidate.canonicalName == hook.canonicalName;
        });
...
static bool ExpandAndValidateHooks(
    ...
    for (const std::string& requestedHook : requestedHooks) {
        if (requestedHook == "all") {
            for (const HookDefinition& availableHook : availableHooks) {
                AddHook(&options->hooks, availableHook);
            }
            continue;
        }

        size_t separator = requestedHook.find('!');
        if (separator == std::string::npos ||
            separator == 0 ||
            separator + 1 == requestedHook.size() ||
            requestedHook.find('!', separator + 1) != std::string::npos) {
            options->error = "Hook names must use module.dll!Export";
            return false;
        }
        std::string canonicalName =
            NormalizeModuleName(requestedHook.substr(0, separator)) +
            requestedHook.substr(separator);
        const HookDefinition* definition = FindSupportedHook(availableHooks, canonicalName);
        if (!definition) {
            options->error = "Unsupported hook: " + requestedHook;
            return false;
        }
        AddHook(&options->hooks, *definition);
    }
...
void PrintUsage() {
    printf("Usage:\n");
    printf("  apiscope.exe [--help | --version | --list-hooks]\n");
    printf("  apiscope.exe run -k <module!export|all> [-k <module!export>] [-f text|jsonl] [-o <path>] [-q] -- <program> [args...]\n");
    printf("  apiscope.exe attach -p <pid> -k <module!export|all> [-k <module!export>] [-f text|jsonl] [-o <path>] [-q]\n");
    printf("\nOptions:\n");
    printf("  -h, --help              Show this help\n");
    printf("  -V, --version           Show version\n");
    printf("  -l, --list-hooks        List supported hooks\n");
    printf("  -p, --pid <pid>         Attach to a running process ID\n");
    printf("  -k, --hook <name|all>   Enable a module-qualified hook\n");
